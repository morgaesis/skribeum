...
$effect(() => {
  ...
  const contentReady =
    ...
    imageDocument !== null ||
    imageError !== null;
  if (path === null || !contentReady || selectedPath !== path) {
    // The focused pane holds no note, canvas, or image: a stale `?note=`
    // from the tab that just closed must not keep naming a document
    // nothing shows any more.
    if (path === null && !contentReady) navigation?.clearAddress();
    return;
  }
