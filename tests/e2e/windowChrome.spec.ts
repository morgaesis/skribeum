// Design system section 4.13: the desktop window draws its own chrome. This
// suite runs on the Linux CI runner, so it exercises everything the Linux
// caption-button code path shares with Windows (geometry, hover fills, the
// danger fill on Close, the fixed button order) plus the platform-neutral
// drag-region and window-state wiring. What it cannot exercise from here:
// the macOS native traffic lights and menu bar, and the Windows 11 snap
// layouts flyout on Maximize hover, both of which need a real session on
// that platform (see the pull request description).
...
describe("desktop window chrome (design system section 4.13)", () => {
  before(async () => {
  ...
  it("reports no system decorations and puts the header at the top of the window", async () => {
    ...
    const geometry = await browser.execute(() => {
      ...
      return {
        ...
        headerTop: header === null ? null : header.getBoundingClientRect().top,
      };
    });
