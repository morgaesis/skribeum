...
- Pull requests run the `rust` and `e2e` jobs on one arm64 Linux runner
  instead of on Linux, macOS and Windows. The full matrix runs on every push
  to `main`, and on a pull request labelled `full-matrix` for a change that
  touches platform-specific behaviour. Both matrices come from one job that
  emits the platform list for the event, so the jobs stay defined once. The
  benchmark regression gate is now its own job, pinned to the x64 runner
  class its committed baselines were measured on.
